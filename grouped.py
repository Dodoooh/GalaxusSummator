...
def sum_amounts(content):
    gesamtbetrag_pattern = r"Gesamtbetrag CHF (\d+)(?:\.\d+)?(?:.–)?"
...
def list_products(content):
    ...
    for order in orders:
        amount = re.search(r"Gesamtbetrag CHF (\d+)(?:\.\d+)?(?:.–)?", order)
        ...
        if amount and date:
            ...
            if num_products > 0 and total_price != 0:
                ...
                for product in products:
                    ...
                    product_dict[name.strip()]['quantity'] += int(quantity)
                    product_dict[name.strip()]['total_price'] += int(quantity) * price_per_product
                    product_dict[name.strip()]['dates'].append(order_date)
...
def export_to_csv(products, file_path):
    with open(file_path, 'w', newline='', encoding='utf-8') as csvfile:
        ...
        for name, product_info in products:
...
def main():
    ...
    for name, product_info in sorted_products:
        quantity = product_info['quantity']
        total_price = product_info['total_price']
        dates = ', '.join(product_info['dates'])
